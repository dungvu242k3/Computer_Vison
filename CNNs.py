...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        correct_test = (predicted_test == Y_test.to(device)).sum().item()

    test_accuracy = 100 * correct_test / total_test
    test_accuracies.append(test_accuracy)

    print(f"Epoch [{epoch+1}/{epochs}], Train Loss: {train_losses[-1]:.4f}, Val Loss: {val_losses[-1]:.4f}, Test Acc: {test_accuracy:.2f}%")
